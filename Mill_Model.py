...
class Mill(object):
    ...
    #----------------------------------
    # --- X Start ---#
    def calc_ode_righthand(self, M_state_tmp, W_state_tmp, SPD):
        ...
        tmp_check = 0
        Ore_RH = np.zeros(len(self.mesh))
        for i in range(0, len(self.mesh)):
            if i == 0:
                # The first size fraction eq 3.1 on pg 24
                # g1w1 - w1(P/M)kE2
                Ore_RH[0] = G_update[0]*np.max(M_state_tmp[0], 0) + np.max(M_state_tmp[0], 0)*(Power_tmp/M_tmp)*kE_update[1]
                tmp_check = tmp_check - np.max(M_state_tmp[0], 0)*(Power_tmp/M_tmp)*kE_update[1]
                
            elif (i > len(self.mesh)-2):
                Ore_RH[i] = G_update[i]*np.max(M_state_tmp[i], 0) -  M_cum_tmp[i]*(Power_tmp/M_tmp)*kE_update[i]
                tmp_check = tmp_check +  M_cum_tmp[i]*(Power_tmp/M_tmp)*kE_update[i]
                
            else:
                # The first size fraction eq 3.4 on pg 26
                # giwi - [Mcumi(P/M)kEi  - (wi + Mcumi)(P/M)kEi+1]
                Ore_RH[i] = G_update[i]*np.max(M_state_tmp[i], 0) - ( np.max(M_cum_tmp[i],0)*(Power_tmp/M_tmp)*kE_update[i] - np.max(M_state_tmp[i] + M_cum_tmp[i],0)*(Power_tmp/M_tmp)*kE_update[i+1] )
                tmp_check = tmp_check + ( np.max(M_cum_tmp[i],0)*(Power_tmp/M_tmp)*kE_update[i] - np.max(M_state_tmp[i] + M_cum_tmp[i],0)*(Power_tmp/M_tmp)*kE_update[i+1] ) 

        if tmp_check > 1e-2:
            print(tmp_check, 'Mass balance problem')
            
        # DERIVATIVES - WATER
        Water_RH = G_update[-1]*np.max(W_state_tmp,0) # W_in (m3/h), Gmax (1/h), Water (tons) density of water is 1 ton/m3

        return Ore_RH, Water_RH
    ...
    def calc_derivative_ore(self, state, t, F, W, SPD):
        ...
        Ore_Out, W_Out = self.calc_ode_righthand(M_state_tmp, W_state_tmp, SPD)
        for i in range(0, len(self.mesh)):
            y[i] =  F[i] - Ore_Out[i]
    ...
    def update_state(self, M_state, Water_state, F_in, W_in, Alpha_spd):
        # This function is only used together with the SS solution where the mill internal states can be explicitly set
    ...
    def grab_measurements(self):
        return np.array([np.ravel(self.Power), np.ravel(self.JT), np.ravel(self.Wc)])
