...
class Sump(object):
    ...
    def calc_level_flow_inf(self, Level_Perc):
        ...
        b = 0.0202
        c = 0.029
        #adjust_factor = (a*Level_Perc**2 + b*Level_Perc + c)*(1 - 1/(np.power(Level_Perc,3) + 1))
        adjust_factor = 1*(1 - 1/(np.power(Level_Perc,1) + 1))
    ...
    def calc_outputs(self, M_state, W_state, SPD, Sump_Perc):
        ...
        # ORE Output
        F_out = np.clip( (Tot_Flow_Out*Vol_Frac)*M_state, 1e-6, 1e10)
        
        # WATER Output
        W_out = np.clip( (Tot_Flow_Out - np.sum(F_out)/self.Ps), 1e-6, 1e10)
    ...
    def grab_measurements(self):
        return np.array([self.Level_Perc, self.P_comb])
    ...
    def historize_data(self, ts):
        ...
        # Mill Start States
        self.HIST_M_state.append(np.ravel(self.M_state)) # Mill Ore holdup per size clas (t) 
        self.HIST_M.append(self.M) # Total Ore holdup (t)
        self.HIST_W_state.append(self.W_state) # Total Water holdup (t)
        self.HIST_F_in.append(np.ravel(self.F_in)) # Ore feed per size class (t/h)
        ...
        self.HIST_F_out.append(np.ravel(self.F_out))
    ...
    def plot_save(self):
        ...
        self.plot_func_2ax([[self.HIST_M, self.HIST_W_state], [np.ravel(self.HIST_dens)]],[['Ore State','Water State'],['Dens']],[['r','b'],['g']])
        plt.savefig(os.getcwd() + '\\Trends\\' + name) 
        plt.close()
        pdf.image(os.getcwd() + '\\Trends\\' + name,w=pdf_W, h=pdf_H)
